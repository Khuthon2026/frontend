...
export default function SearchBar({ query, setQuery, onUrlClick }: Props) {
  const [open, setOpen] = useState(false);
  const [activeIdx, setActiveIdx] = useState(-1);
  const wrapRef = useRef<HTMLDivElement>(null);

  const q = query.trim().toLowerCase();
  const results = q ? APPS.filter((a) => a.name.toLowerCase().includes(q)).slice(0, 6) : [];
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      e.preventDefault();
      setQuery(results[activeIdx].name);
      setOpen(false);
      ...
      {open && results.length > 0 && (
        <div className="absolute left-0 right-0 top-[calc(100%+10px)] z-10 max-h-[380px] animate-drop-in overflow-y-auto rounded-[22px] bg-white/[0.98] p-2.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.4),0_24px_60px_rgba(0,0,0,0.4),0_0_0_1px_rgba(255,255,255,0.08)] backdrop-blur">
          ...
          {results.map((a, i) => (
            ...
            >
              <div
                className="font-product grid h-10 w-10 flex-shrink-0 place-items-center rounded-[10px] text-lg font-bold text-white"
              ...
              <div className="min-w-0 flex-1">
                <div
                  ...
                  dangerouslySetInnerHTML={{ __html: highlight(a.name, query.trim()) }}
                />
              </div>
            </div>
          ))}
